...
def check(url):
    if asyncio.run(web_check.site_is_online(url)) is True:
        showinfo(
            title="Site online",
            message=f"{url} is online. If your browser doesn't display the page, please check the browser configuration.",
        )
    else:
        showerror(
            title="Site offline",
            message=f"{url} appears to be offline. Please try again later.",
        )
...
def error_message(url):
    if web_check.check_site_address(url) is None:
        showwarning(
            ...
            message=f"To test a connection with a site, you must provide a URL. {url} is not a valid url.",
        )
    if web_check.check_site_address(url):
        ...
        if web_check.check_site(url) is False:
            showerror(
                title="Site nonexistent",
                message=f"Unfortunately, the {url} entered is not a valid website. Please try again.",
            )
        else:
            check(url)
...
def get_help():
    ...
    webbrowser.open("feature/help_section.md")
...
def main():
    ...
    # app banner
    ttk.Label(
        text="Check Any URL Connection",
        anchor="n",
        justify=CENTER,
        font=("Helvetica", 20, "bold"),
    ).grid(row=1, rowspan=2, column=0, columnspan=3, pady=25)
    ...
    ttk.Button(
        ...
        command=lambda: enable_partial_url(
            partial_bool.get(), ui_url.get(), mute=False
        ),
    ...
    partial = ttk.Checkbutton(
        root,
        text="Autocomplete URL elements",
        variable=partial_bool,
        onvalue=True,
        offvalue=False,
        command=lambda: enable_partial_url(partial_bool.get(), ui_url.get(), mute=True),
    )
    partial.grid(row=4, columnspan=3, pady=15, sticky="n")

    # speed test button implementation
    ...
    ttk.Button(
        root,
        text="Check Internet Speed",
        command=lambda: web_check.get_speed(down_speed, up_speed),
